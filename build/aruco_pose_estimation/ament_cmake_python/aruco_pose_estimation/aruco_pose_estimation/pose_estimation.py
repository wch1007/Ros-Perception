...
def pose_estimation(rgb_frame: np.array, depth_frame: np.array, aruco_detector: cv2.aruco.ArucoDetector, marker_size: float,
    ...
    Returns:
        frame - image with bounding boxes and axes drawn
        pose_array - PoseArray to publish
        markers - ArucoMarkers message to publish
    '''
    ...
    corners, marker_ids, rejected = aruco_detector.detectMarkers(gray)
    print(f"[DEBUG] Marker IDs: {marker_ids}")
    print(f"[DEBUG] Image size: {rgb_frame.shape}")

    # Save image for debugging
    cv2.imwrite("/tmp/debug_frame.png", rgb_frame)
    ...
    if marker_ids is not None and len(corners) > 0:
        for i, marker_id in enumerate(marker_ids):
            ...
            # Draw marker boundaries and coordinate axes
...
def my_estimatePoseSingleMarkers(corners, marker_size, camera_matrix, distortion) -> tuple[np.array, np.array, np.array]:
    ...
    retval, rvec, tvec = cv2.solvePnP(objectPoints=marker_points,
                                      imagePoints=corners,
    ...
    quaternion = mat2quat(rot_matrix)
